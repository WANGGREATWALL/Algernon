...
int convertCLImageFormat2NTIFormat(const cl_image_format& fmtCL, int& fmtCV)
{
    ...
    XCHECK_WITH_RET(bit > 0, err::kErrorInvalidParam);
...
bool isValid(const cl::Image& image) { return getImageWidth(image) > 0 && getImageHeight(image) > 0; }
...
int CLWrapper::init(bool enableProfiling)
{
    ...
    XCHECK_WITH_RET(!mFolderBinary.empty(), err::kErrorInvalidParam);
    XCHECK_WITH_RET(file::isDirectory(mFolderBinary), err::kErrorInvalidParam);
    XCHECK_WITH_RET(!mNameBinaryWithoutFormat.empty(), err::kErrorInvalidParam);
...
int CLWrapper::build(const std::string& kernelString, const std::string optionsCompile)
{
    ...
    XCHECK_WITH_RET(!kernelString.empty(), err::kErrorInvalidParam);
    XCHECK_WITH_RET(!optionsCompile.empty(), err::kErrorInvalidParam);
    XCHECK_WITH_RET(!mFolderBinary.empty(), err::kErrorInvalidParam);
    XCHECK_WITH_RET(file::isDirectory(mFolderBinary), err::kErrorInvalidParam);
    XCHECK_WITH_RET(!mNameBinaryWithoutFormat.empty(), err::kErrorInvalidParam);
    ...
    if (loadCheckKey(mKeyLoaded) == err::kSuccess) {
        checkLoadKey(mKeyLoaded, isBinAvailable);
    }

    if (isBinAvailable) {
        int retBuildProgram = buildProgram(optionsCompile);
        XCHECK_WITH_RET(retBuildProgram == CL_SUCCESS, retBuildProgram);
    } else {
        int retCreateProgram = createProgramWithKernelString(mStringKernel);
        XCHECK_WITH_RET(retCreateProgram == CL_SUCCESS, retCreateProgram);

        int retBuildProgram = buildProgram(optionsCompile);
        XCHECK_WITH_RET(retBuildProgram == CL_SUCCESS, retBuildProgram);

        int retSaveBinAndKey = storeBinAndKey();
        XCHECK_WITH_RET(retSaveBinAndKey == err::kSuccess, retSaveBinAndKey);
    }
...
int CLWrapper::createKernels()
{
    ...
    XCHECK_WITH_MSG(retCreateKernels == CL_SUCCESS, retCreateKernels, "clError: %s",
                    clErrorInfo(retCreateKernels).c_str());
...
{
    XCHECK_WITH_RET(cv::isValid(image), err::kErrorInvalidParam);
    XCHECK_WITH_RET(
        cv::isFormatIn(image, {cv::kXFormatGrayU8, cv::kXFormatGrayU16, cv::kXFormatGrayU32, cv::kXFormatRGBU8,
                               cv::kXFormatBGRU8, cv::kXFormatRGBAU8, cv::kXFormatBGRAU8}),
...
{
    XCHECK_WITH_RET(data != nullptr, err::kErrorInvalidParam);
...
int CLWrapper::createImage2D(cl::Image2D& dst, void* data, int width, int height, cl::ImageFormat format,
                             cl_mem_flags flags)
{
    XCHECK_WITH_RET(data != nullptr, err::kErrorInvalidParam);
    XCHECK_WITH_RET(width > 0 && height > 0, err::kErrorInvalidParam);
...
int CLWrapper::createImage3D(cl::Image3D& dst, const cv::Image& image, cl::ImageFormat format, cl_mem_flags flags)
{
    ...
    XCHECK_WITH_RET(
        cv::isFormatIn(image, {cv::kXFormatGrayU8, cv::kXFormatGrayU16, cv::kXFormatGrayU32, cv::kXFormatRGBU8,
                               cv::kXFormatBGRU8, cv::kXFormatRGBAU8, cv::kXFormatBGRAU8}),
        ERROR_UNSUPPORTED_TYPE);
    XCHECK_WITH_RET(image.width * image.width == image.height, err::kErrorInvalidParam);
...
int CLWrapper::createImage3D(cl::Image3D& dst, void* data, int width, int height, int depth, cl::ImageFormat format,
                             cl_mem_flags flags)
{
    XCHECK_WITH_RET(data != nullptr, err::kErrorInvalidParam);
...
int CLWrapper::createImage3D(cl::Image3D& dst, int width, int height, int depth, cl::ImageFormat format,
                             cl_mem_flags flags)
{
    int ret = CL_SUCCESS;
...
{
    XCHECK_WITH_RET(isValid(src), err::kErrorInvalidParam);
    XCHECK_WITH_RET(isSameSizeAndFormat(src, dst), err::kErrorInvalidParam);
...
int CLWrapper::readImage2D(cv::Image& dst, const cl::Image2D& image, bool block)
{
    ...
    XCHECK_WITH_RET(cv::isValid(dst), err::kErrorInvalidParam);
    XCHECK_WITH_RET(isSize(image, dst.width, dst.height), err::kErrorInvalidParam);
...
int CLWrapper::readImage2D(cv::Image& dst, const cl::Image2D& plane0, const cl::Image2D& plane1)
{
    ...
    XCHECK_WITH_RET(cv::isValid(dst), err::kErrorInvalidParam);
    XCHECK_WITH_RET(cv::isFormatIn(dst, {cv::kXFormatNV12, cv::kXFormatNV21}), err::kErrorInvalidParam);

    cl_int ret = CL_SUCCESS;

    trace.sub("query");
    XCHECK_WITH_RET(isSize(plane0, dst.width, dst.height), err::kErrorInvalidParam);
    XCHECK_WITH_RET(isSize(plane1, dst.width / 2, dst.height / 2), err::kErrorInvalidParam);
    ...
    trace.sub("read");
    cl::Event event;
    int       retReadImage0 = mCommandQueue.enqueueReadImage(plane0, false, {0, 0, 0},
                                                             {(cl::size_type)dst.width, (cl::size_type)dst.height, 1}, pitch0,
                                                             0, dst.data[0], 0, &event);
    ...
    int retReadImage1 = mCommandQueue.enqueueReadImage(
        plane1, false, {0, 0, 0}, {(cl::size_type)dst.width >> 1, (cl::size_type)dst.height >> 1, 1}, pitch1, 0,
        dst.data[1], 0, &event);
...
int CLWrapper::readImage2D(void* dst, const cl::Image2D& image, int width, int height, int pitch, bool block)
{
    ...
    XCHECK_WITH_RET(dst != nullptr, err::kErrorInvalidParam);

    cl::Event event;
    int       retReadImage =
        mCommandQueue.enqueueReadImage(image, block, {0, 0, 0}, {(cl::size_type)width, (cl::size_type)height, 1},
                                       (cl::size_type)pitch, 0, dst, 0, &event);
    XCHECK_WITH_MSG(retReadImage == CL_SUCCESS, retReadImage, "clError: %s", clErrorInfo(retReadImage).c_str());
    mEvents.emplace_back("readImage2D", event);
...
int CLWrapper::mapImage2D(cv::Image& dst, const cl::Image2D& image)
{
    ...
    XCHECK_WITH_RET(cv::isValid(dst), err::kErrorInvalidParam);
    XCHECK_WITH_RET(cv::isFormatIn(dst, {cv::kXFormatGrayU8, cv::kXFormatRGBU8, cv::kXFormatBGRU8, cv::kXFormatRGBAU8,
                                         cv::kXFormatBGRAU8}),
                    err::kErrorInvalidParam);
    ...
    XCHECK_WITH_RET(dst.height == height, err::kErrorInvalidParam);
    XCHECK_WITH_RET(dst.width <= pitch, err::kErrorInvalidParam);
...
int CLWrapper::mapImage2D(cv::Image& dst, const cl::Image2D& plane0, const cl::Image2D& plane1)
{
    ...
    XCHECK_WITH_RET(pitch0 == pitch1, err::kErrorInvalidParam);
    XCHECK_WITH_RET(width0 == width1 * 2, err::kErrorInvalidParam);
    XCHECK_WITH_RET(height0 == height1 * 2, err::kErrorInvalidParam);
    XCHECK_WITH_RET(dst.width == width0 && dst.height == height0 && dst.stride[0] == pitch0, err::kErrorInvalidParam);
...
int CLWrapper::getDevice()
{
    ...
    XCHECK_WITH_RET(!mPlatforms.empty(), err::kErrorInvalidParam);
...
int CLWrapper::createContext(cl_context_properties* properties)
{
    ...
    XCHECK_WITH_RET(!mDevices.empty(), err::kErrorInvalidParam);
...
int CLWrapper::createProgramWithKernelString(const std::string& kernel)
{
    ...
    if (ret != CL_SUCCESS) {
        ...
        XCHECK_WITH_MSG(retGetProgramBuildLog == CL_SUCCESS, retGetProgramBuildLog, "clError: %s",
...
{
    XCHECK_WITH_MSG(mKernels.find(name) != mKernels.end(), ERROR_NOT_FOUND, "failed to find kernel[%s]!", name.c_str());
...
void CLWrapper::checkLoadKey(const std::string& key, bool& isBinAvailable)
{
    ...
    if (key.size() == 60) {
        ...
        if (getDeviceKey(mKeyDevice) == err::kSuccess && mKeyDevice == keyDeviceLoaded) {
            LOGGER_D("CLWrapper: key verification(2/5)\n");

            if (getKernelKey(mKeyKernel) == err::kSuccess && mKeyKernel == keyKernelLoaded) {
                LOGGER_D("CLWrapper: key verification(3/5)\n");

                if (getBinaryKey(mKeyBinary) == err::kSuccess && mKeyBinary == keyBinaryLoaded) {
                    LOGGER_D("CLWrapper: key verification(4/5)\n");

                    std::vector<unsigned char> bin(mStringBinary.cbegin(),
                                                   mStringBinary.cend());  // mStringBinary loaded in getBinaryKey()
...
int CLWrapper::loadCheckKey(std::string& key)
{
    ...
    XCHECK_WITH_RET(retLoadBuffer == err::kSuccess, retLoadBuffer);
...
int CLWrapper::storeBinAndKey()
{
    ...
    XCHECK_WITH_RET(!mFolderBinary.empty(), err::kErrorInvalidParam);
    XCHECK_WITH_RET(!mNameBinaryWithoutFormat.empty(), err::kErrorInvalidParam);
    XCHECK_WITH_RET(!mStringKernel.empty(), err::kErrorInvalidParam);
    ...
    XCHECK_WITH_MSG(retGetBinary == CL_SUCCESS && !bins.empty(), retGetBinary, "clError: %s",
                    clErrorInfo(retGetBinary).c_str());

    std::string bin(bins.front().cbegin(), bins.front().cend());

    int retSaveBin = file::XFile::saveBufferToFile(bin, pathBin);
    XCHECK_WITH_RET(retSaveBin == err::kSuccess, retSaveBin);

    trace.sub("key");
    int retGetDeviceKey = getDeviceKey(mKeyDevice);
    XCHECK_WITH_RET(retGetDeviceKey == err::kSuccess, retGetDeviceKey);
    ...
    XCHECK_WITH_RET(retSaveKey == err::kSuccess, retSaveKey);
...
int CLWrapper::getDeviceKey(std::string& key)
{
    ...
    XCHECK_WITH_RET(retGetName == err::kSuccess, retGetName);

    int retGetVendor = getDeviceVendor(vendorDevice);
    XCHECK_WITH_RET(retGetVendor == err::kSuccess, retGetVendor);

    int retGetVersion = getDeviceVersion(versionDevice);
    XCHECK_WITH_RET(retGetVersion == err::kSuccess, retGetVersion);
...
int CLWrapper::getBinaryKey(std::string& key)
{
    ...
    XCHECK_WITH_RET(retLoadBuffer == err::kSuccess, retLoadBuffer);
    XCHECK_WITH_RET(!mStringBinary.empty(), ERROR_INVALID_DATA);
